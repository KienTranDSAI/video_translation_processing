...
if __name__ == "__main__":
    first = (cv2.imread("first.png")/255).astype(np.float32)
    
    second = (cv2.imread("second.png")/255).astype(np.float32)
    # first = cv2.resize(first, (second.shape[1], second.shape[0]))
    second = cv2.resize(second, (first.shape[1], first.shape[0]))
    test = first[300:370,200:330,:]
    test1 = second[170:240,180:310,:]
    newSource = color_transfer("mkl", test, test1)
    # newSource = color_transfer_sot(first,second, steps=10, batch_size=5, reg_sigmaXY=16.0, reg_sigmaV=5.0)
    # newSource = color_transfer_mkl(first, second)
    # newSource = color_transfer_idt(first, second, bins=256, n_rot=20)
    # cv2.imshow("Image",first)
    # cv2.imshow("Image",second)

    cv2.imshow("Image",newSource)
